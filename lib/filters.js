...
var filters = {
  ...
  lomo: function(req, res) {
    ...
    var key = uuid.v4().split('-').pop(),
      source = imagePath + req.key + '.jpg',
      target = imagePath + key + '.jpg',
      args = [source, '-fill', '#222b6d', '-colorize', '30','-modulate', '90,80,10', '-compose', 'Over', target],
      convert = spawn('convert', args);
  ...
  gotham: function(req, res) {
    var key = uuid.v4().split('-').pop(),
      source = imagePath + req.key + '.jpg',
      target = imagePath + key + '.jpg',
      args = [source, '-modulate', '120,10,100', '-fill', '222b6d', '-colorize', '20', '-gamma', '0.5', '+contrast', '+contrast', '-compose', 'Over', 
      'borderColor', 'black', '-border', '7,7', target],
      convert = spawn('convert', args);
